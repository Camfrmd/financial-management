...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 flex justify-between items-end">
        <div>
            <h1 class="text-2xl font-bold text-white mb-2">{{ __('Edit Fund') }}: {{ $fund->name }}</h1>
            <p class="text-gray-400">{{ __('Update the details of this community fund. The balance cannot be changed manually.') }}</p>
        </div>
        <div class="bg-gray-800/50 border border-gray-700/50 rounded-lg px-4 py-2">
            <span class="text-sm text-gray-500 block">{{ __('Current Balance') }}</span>
            <span class="text-xl font-bold {{ $fund->current_balance >= 0 ? 'text-green-500' : 'text-red-500' }}">Rp {{ number_format($fund->current_balance, 0, ',', '.') }}</span>
        </div>
    </div>
    ...
    <div class="bg-[#1a1d2d] rounded-xl border border-gray-800 shadow-xl overflow-hidden">
        <div class="p-6 sm:p-8">
            <form action="{{ route('funds.update', $fund->fund_id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $fund->name) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors">{{ old('description', $fund->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="group_id" id="group_id"
                            ...
                            @foreach($groups as $group)
                                <option value="{{ $group->group_id }}" {{ old('group_id', $fund->group_id) == $group->group_id ? 'selected' : '' }}>
                                    {{ $group->group_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="activity_id" id="activity_id"
                            ...
                            @foreach($activities as $activity)
                                <option value="{{ $activity->activity_id }}" {{ old('activity_id', $fund->activity_id) == $activity->activity_id ? 'selected' : '' }}>

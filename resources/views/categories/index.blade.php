                                    ...
                                    @foreach($category->children as $child)
                                        <div class="flex justify-between items-center group">
                                            ...
                                            <div class="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <a href="{{ route('categories.edit', $child->category_id) }}" class="text-xs text-gray-500 hover:text-white transition-colors">{{ __('Edit') }}</a>
                                                <form action="{{ route('categories.destroy', $child->category_id) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-xs text-gray-500 hover:text-red-500 transition-colors">{{ __('Delete') }}</button>
                                                </form>
                                            </div>
                                        </div>
                                    @endforeach
